...
mat_long <- pivot_longer(data = mat, cols = BM_WT_pBM5_1_S29.1:BM_enp1_1_pBM766_3_S46.140, 
...
# make a plasmid column
mat_long$plasmid <- NA
mat_long$plasmid <- ifelse(grepl(pattern = "pBM5", mat_long$sample), "pBM5", mat_long$plasmid)
mat_long$plasmid <- ifelse(grepl(pattern = "pBM766", mat_long$sample), "pBM766", mat_long$plasmid)

mat_grouped <- mat_long %>%
  group_by(bin, strain, plasmid) %>%
  summarize(mean_rpkm = mean(rpkm))

mat_grouped$bin <- as.numeric(mat_grouped$bin)

pdf(snakemake@output[['plt']], height = 4, width = 7)
ggplot(mat_grouped, aes(x = bin, y = mean_rpkm, color = strain, linetype = plasmid)) +
  geom_line(size=1.2) + 
